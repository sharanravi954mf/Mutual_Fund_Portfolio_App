...
abstract class FolioVerificationLogSink {
  ...
  void operationFailed(String operation, String correlationId,
      FolioVerificationApplicationFailure failure);
}
...
enum FolioVerificationApplicationFailureCode {
  unauthenticated,
  permissionDenied,
  unavailable,
  invalidTransition,
...
class FolioVerificationApplicationFailure implements Exception {
  const FolioVerificationApplicationFailure(this.code);
  final FolioVerificationApplicationFailureCode code;
}

class SubmitFolioVerificationCommand {
  const SubmitFolioVerificationCommand(
      {required this.token,
      required this.relationship,
      required this.correlationId});
  final FolioSubmissionToken token;
  final FolioHolderRelationship relationship;
  final String correlationId;
}

class FolioVerificationDecisionCommand {
...
class RevokeFolioGrantCommand {
  const RevokeFolioGrantCommand(
      {required this.grantId,
      required this.expectedVersion,
      required this.reasonCode,
      required this.correlationId});
  final String grantId, reasonCode, correlationId;
  final int expectedVersion;
}

class FolioVerificationService {
  FolioVerificationService(this._investorRepository, this._advisorRepository,
      {FolioVerificationLogSink logSink = const NoopFolioVerificationLogSink()})
      : _logSink = logSink;
  ...
  Future<FolioVerificationRequest> resubmit(
      ...
      _run(
          'resubmit',
          command.correlationId,
          () => _investorRepository.resubmit(command.requestId,
              command.expectedVersion, command.correlationId));
  Future<FolioVerificationRequest> cancel(
          FolioVerificationDecisionCommand command) =>
      _run(
          'cancel',
          command.correlationId,
          () => _investorRepository.cancel(command.requestId,
              command.expectedVersion, command.correlationId));
  Future<FolioVerificationRequest> beginReview(
      ...
      _run(
          'beginReview',
          command.correlationId,
          () => _advisorRepository.beginReview(command.requestId,
              command.expectedVersion, command.correlationId));
  ...
  Future<T> _run<T>(String operation, String correlationId,
    ...
    try {
      ...
      _logSuccess(operation, correlationId);
      return result;
    } catch (error) {
      final failure = _map(error);
      _logFailure(operation, correlationId, failure);
      throw failure;
    }
  }

  void _logSuccess(String operation, String correlationId) {
    try {
      _logSink.operationSucceeded(operation, correlationId);
    } catch (_) {}
  }

  void _logFailure(String operation, String correlationId,
      FolioVerificationApplicationFailure failure) {
    try {
      _logSink.operationFailed(operation, correlationId, failure);
    } catch (_) {}
  }
  ...
  FolioVerificationApplicationFailure _map(Object error) {
    if (error is FolioVerificationApplicationFailure) return error;
    if (error is FolioVerificationFailure) {
      return FolioVerificationApplicationFailure(switch (error.code) {
        FolioVerificationFailureCode.unauthenticated =>
        ...
        FolioVerificationFailureCode.requestUnavailable =>
          FolioVerificationApplicationFailureCode.unavailable,
        FolioVerificationFailureCode.invalidTransition =>
          FolioVerificationApplicationFailureCode.invalidTransition,
        FolioVerificationFailureCode.staleVersion =>
          FolioVerificationApplicationFailureCode.staleVersion,
        FolioVerificationFailureCode.duplicateRequest ||
        FolioVerificationFailureCode.duplicateGrant =>
          FolioVerificationApplicationFailureCode.duplicate,
        FolioVerificationFailureCode.tokenInvalidOrExpired =>
          FolioVerificationApplicationFailureCode.tokenInvalidOrExpired,
        FolioVerificationFailureCode.evidenceChanged =>
          FolioVerificationApplicationFailureCode.evidenceChanged,
        FolioVerificationFailureCode.unsupportedRelationship =>
          FolioVerificationApplicationFailureCode.unsupportedRelationship,
        FolioVerificationFailureCode.validationFailed =>
          FolioVerificationApplicationFailureCode.validation,
        FolioVerificationFailureCode.timeout =>
          FolioVerificationApplicationFailureCode.timeout,
        FolioVerificationFailureCode.temporaryFailure =>
    ...
    return const FolioVerificationApplicationFailure(
        FolioVerificationApplicationFailureCode.unexpected);
  }
}
