...
class _ClientDashboardState extends State<ClientDashboard> {
  ...
  late final PortfolioRepository _portfolioRepository =
      widget.portfolioRepository ??
          SupabasePortfolioRepository(Supabase.instance.client);
  final PortfolioDashboardService _dashboardService =
  ...
  void _refreshData() {
    setState(() {
      _portfolioDataFuture =
          _portfolioRepository.loadCurrentInvestorPortfolio();
    });
  }
  ...
  Future<void> _performFundSearch(String query) async {
    ...
    try {
      ...
      if (isNumeric && cleanQuery.length >= 5 && cleanQuery.length <= 6) {
        ...
        results = await _fundSearchService.search(cleanQuery);
      }
    ...
    return FutureBuilder<InvestorPortfolioData>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          final isDenied = snapshot.error is PortfolioAccessDeniedException;
          return Scaffold(
            backgroundColor: colors.background,
            body: Center(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Icon(
                      isDenied ? Icons.lock_outline : Icons.cloud_off_outlined,
                      color: isDenied ? colors.primary : colors.error,
                      size: 44,
                    ),
                    const SizedBox(height: 16),
                    Text(
                      isDenied
                          ? 'Portfolio access is unavailable'
                          : 'We could not load your portfolio',
                    ...
                    Text(
                      isDenied
                          ? 'Your investment account is not currently linked. Please contact your advisor if you need help.'
                          : 'Please try again. If the problem continues, contact your advisor.',
                      textAlign: TextAlign.center,
        ...
        final dashboard = _dashboardService.build(data);
        final allFunds = data.allFunds;

        final clientName = dashboard.investorName != null &&
                dashboard.investorName!.trim().isNotEmpty
            ? dashboard.investorName!
                      ...
                      children: [
                          ...
                          child: ListView(
                            ...
                            children: [
                              ...
                              _buildDrawerItem(3, t('about_us_nav'),
                                  Icons.info_outline, colors, context),
                              _buildDrawerItem(
                                  4,
                                  t('contact_us'),
                        ...
                        Padding(
                          ...
                          child: InkWell(
                            ...
                            .animate(
                                delay: const Duration(milliseconds: 6 * 80))
                            .fadeIn(
          ...
          body: RupeeRainBackground(
            child: Column(
              children: [
                ...
                _buildTopHeaderBar(
                    colors, t, appBarTitle, clientName, user, authProvider),
                Expanded(
                  child: Row(
                    children: [
                      _buildDesktopSidebar(
                          colors, t, clientName, user, authProvider),
                      Expanded(
                        child: SafeArea(
                          child: tabContent,
                        ).animate().fadeIn(
  ...
  Widget _buildTopHeaderBar(
    ...
    final greeting = clientName.isNotEmpty
        ...
        children: [
          ...
          Expanded(
            child: ListView(
              padding: EdgeInsets.zero,
              children: [
                _buildSidebarItem(0, t('portfolio'),
                ...
                _buildSidebarItem(
                    3, t('about_us_nav'), Icons.info_outline, colors),
                _buildSidebarItem(
                    4, t('contact_us'), Icons.contact_support_outlined, colors),
              ],
            ),
          ),
  ...
  Widget _buildPortfolioContent({
    required PortfolioDashboardData dashboard,
      ...
      return Center(
        child: Padding(
          ...
          child: _buildEmptyStateCard(
            colors,
            'Your portfolio is being prepared',
            'No portfolio data is available yet. Your advisor will update it after processing your registrar statement.',
            Icons.account_balance_wallet_outlined,
          ),
        ),
      );
        ...
        child: Column(
          ...
          children: [
            ...
            if (dashboard.warnings.isNotEmpty) ...[
              const SizedBox(height: 16),
              _buildPortfolioNotice(colors, dashboard.warnings.first),
            ],
                    ...
                    children: [
                      Expanded(
                          child: _buildMetricCard(
                                  colors,
                                  t('total_invested'),
                                  currencyFormat.format(summary.investedAmount),
                                  Icons.account_balance_wallet_outlined,
                      ...
                      Expanded(
                          child: _buildMetricCard(
                                  colors,
                                  'Gain / Loss',
                                  currencyFormat.format(summary.gainLoss),
                                  Icons.pie_chart_outline,
                                  const Color(0xFFF27121),
                                  isReturn: true,
                                  returnValue: summary.gainLoss)
                      ...
                      Expanded(
                          child: _buildMetricCard(
                                  colors,
                                  'Return',
                                  _percentage(summary.returnPercentage),
                                  Icons.percent_outlined,
                                  const Color(0xFF00C853),
                                  isReturn: true,
                                  returnValue: summary.returnPercentage ?? 0)
            ...
            (dashboard.holdings.isEmpty
                    ...
                    : Container(
                        ...
                        child: ListView.separated(
                          ...
                          itemCount: dashboard.holdings.length,
                          separatorBuilder: (context, index) =>
                              Divider(color: colors.border, height: 1),
                          itemBuilder: (context, index) {
                            final h = dashboard.holdings[index];
                            ...
                            return Material(
                              ...
                              child: ListTile(
                                ...
                                subtitle: Padding(
                                  ...
                                  child: Text(
                                    "${h.units.toStringAsFixed(4)} Units  •  NAV: ₹${h.nav.toStringAsFixed(2)}",
                                    style: GoogleFonts.inter(
                                        color: colors.textSecondary,
                                        fontSize: 12),
                                  ),
                                ...
                                onTap: () {
                                  showDialog(
                                    ...
                                    builder: (context) => FactsheetDialog(
                                      ...
                                      category: h.category ?? 'Not classified',
                                      fundHouse:
                                          h.fundHouse ?? 'Not classified',
                                    ),
                                  );
                                },
                              ),
            ...
            _buildAllocationSection(
              colors: colors,
              title: 'Allocation',
              amcAllocation: dashboard.amcAllocation,
              categoryAllocation: dashboard.categoryAllocation,
                            ...
                            return Material(
                              ...
                              child: ListTile(
                                ...
                                subtitle: Padding(
                                  padding: const EdgeInsets.only(top: 4.0),
                                  child: Text(
                                    "${dateFormat.format(tx.executionDate)}  •  ${tx.units.toStringAsFixed(4)} Units",
                                    style: GoogleFonts.inter(
                                        color: colors.textSecondary,
                                        fontSize: 11),
                                  ),
                                ...
                                trailing: Text(
                                  "${isBuy ? '+' : '-'}${currencyFormat.format(tx.amount)}",
                                  style: GoogleFonts.outfit(
                                    color: isBuy
                                        ? const Color(0xFF00C853)
                                        : const Color(0xFFFF1744),
  ...
  Widget _buildPortfolioNotice(AppThemeColors colors, String message) {
    return Container(
  ...
  Widget _buildAllocationSection({
    ...
    required List<AllocationSummary> amcAllocation,
    required List<AllocationSummary> categoryAllocation,
  }) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            final cards = [
              _buildAllocationCard(colors, 'By AMC', amcAllocation),
              _buildAllocationCard(colors, 'By category', categoryAllocation),
            ];
  ...
  Widget _buildAllocationCard(
    AppThemeColors colors,
    String title,
    List<AllocationSummary> allocation,
  ) {
    return Container(
      width: double.infinity,
      ...
      child: Column(
        ...
        children: [
          ...
          if (allocation.isEmpty)
            Text(
              'Allocation is unavailable until active holdings are available.',
              style:
                  GoogleFonts.inter(color: colors.textSecondary, fontSize: 12),
            )
          else
              ...
              (item) => Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Semantics(
                      label: '${item.label}: ${_percentage(item.percentage)}',
  ...
  Widget _buildSelectedFundCard(
      AppThemeColors colors, String Function(String) t) {
        ...
        Row(
          ...
          children: [
            Text(
              firstDate,
              style:
                  GoogleFonts.inter(color: colors.textSecondary, fontSize: 10),
            ),
            Text(
