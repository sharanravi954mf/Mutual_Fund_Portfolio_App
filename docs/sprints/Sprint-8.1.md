...
- [x] **feat(database/flutter): complete Investor referral mechanics with caller-bound idempotent code creation, atomic concurrency-safe conversion and 30-day reward entitlements, immutable reward audit evidence, exclusive profile-resolved RLS, exact API grants, and encoded WhatsApp sharing [BRD-FR-011, Section 12]** (#40)
