...
- [x] **feat(database/flutter): complete Investor referral mechanics with caller-bound idempotent code creation, `/join?ref=` web/Android onboarding attribution, server-enforced 24-hour claim TTL lifecycle, service-role batch cleanup with partial index, exactly-once post-profile conversion, atomic concurrency-safe conversion, server-timed 30-day Premium Investor entitlement authorization for both participants, immutable reward audit evidence, exclusive profile-resolved RLS, exact API grants, and encoded WhatsApp sharing [BRD-FR-011, Section 12, Section 13]** (#40)
