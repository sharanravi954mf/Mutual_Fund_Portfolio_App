...
- [ ] **feat(database): create order_requests and workspace_audit_logs tables [BRD-FR-005, BRD-FR-006, BRD-BR-005, BRD-NFR-005, Section 5, Section 6.D]** (#29)
- [ ] **feat(database): implement workspace isolation RLS policy on order_requests [BRD-BR-003, Section 6.A]** (#30)
- [ ] **feat(database): implement platform admin override and family delegation RLS policies [BRD-BR-007, BRD-BR-009, Section 6.F]** (#31)
- [ ] **feat(edge): build Deno in-memory stream parser worker for CAMS/KFintech feeds [BRD-BC-006, BRD-FR-009, Section 7.A]** (#32)
- [ ] **feat(edge): build Order Auto-Approval RPC Engine [BRD-BR-006, Section 6.B, Section 7.B]** (#33)
...
- [ ] **test(database): add pgTAP automated tests for RLS workspace isolation and auto-approval triggers [BR-003, BR-006, Section 6.A, Section 6.B]** (#41)
...
- [ ] **feat(database): create auto_approval_rules table and decision evaluation triggers [BRD-BR-006, Section 6.B]** (#50)
- [ ] **test(database): add pgTAP automated unit tests for qualify_order, cancel_order, and outbox triggers [BRD-BR-003, BRD-FR-005, Section 6.A, Section 6.B]** (#51)
- [ ] **feat(database): create plan_entitlements table for feature gating [BRD-BR-011, Section 13]** (#53)
- [ ] **feat(database): create service-only apply_auto_approval_decision RPC [BRD-FR-005, Section 6.C]** (#59)
