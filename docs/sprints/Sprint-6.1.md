...
- [ ] **feat(database): create order_requests and immutable workspace_audit_logs tables (covers investor self-initiation and distributor-assisted initiation capturing investor_profile_id, initiated_by_profile_id, initiated_by_role, target workspace_id, and initiation_channel with immutable audit trails) [BRD-FR-005, BRD-FR-006, BRD-BR-005, BRD-NFR-005, Section 5, Section 6.A, Section 17.F]** (#29)
- [ ] **feat(database): implement workspace isolation RLS policy on order_requests (enforces same-workspace relationship validation for both self-initiated and distributor-assisted orders, ensuring no cross-workspace access) [BRD-BR-003, Section 6.A]** (#30)
...
- [ ] **feat(flutter): build Investor Order Request Modal and Distributor-assisted Order Initiation Workflow (Buy/Sell/Switch) [BRD-FR-005, Section 3.A]** (#34)
...
- [ ] **test(database/edge): add pgTAP and service-level tests for qualify_order, cancel_order, order initiation, immutable audits, and Platform Admin overrides (includes tests for distributor-assisted initiation, same-workspace validation, initiator identity persistence, and negative tests for unrelated MFDs, Platform Admins, and Family Guests) [BRD-BR-003, BRD-BR-007, BRD-FR-005, Section 6.D, Section 6.E, Section 6.G, Section 17.F]** (#51)
...
- [ ] **docs/implementation: complete Sprint 6.1 canonical alignment (covering canonical documents, migration contracts, issue scopes, test evidence, and project board synchronization) [Section 1, Section 18]** (#60)