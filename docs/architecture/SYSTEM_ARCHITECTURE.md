...
* **Mailbag & Feed Ingestion Worker**:
  * Receives CAMS/KFintech DBF feeds. CAMS WBR2/WBR9 Gmail mailbacks are
    supported; WBR49 remains explicitly out of scope. The PDF service/Edge contract
...
```text
[OAuth Connector] ➔ [Poll Mailbox] ➔ [Validate Sender]
  ➔ [Attachment OR validated CAMS URL ➔ encrypted ZIP ➔ DBF]
  ➔ [Hash + MIME/Magic-Byte Check] ➔ [Malware Scan] ➔ [Encrypted Object Storage]
  ➔ [Memory Stream Parser] ➔ [Immutable Ingestion Log]
```
...
* **Sanitized Support Diagnostics**: Raw Uvicorn access logs remain disabled. The support API emits fixed-schema INFO JSON lines through one non-propagating stdout handler: safe request route/status/duration, successful mailbox message/attachment counts, and sanitized ServiceError code/status. Three bounded Gmail poll failures may add only an internal enum-backed reason distinguishing list response size, individual detail response size, or attachment-count overflow. The reason is never returned by the public API. Request/response bodies, headers, provider identities, filenames, senders, subjects, URLs/query strings, content, and credentials are excluded.
* **Mailbox Backlog Bounds**: Generic Gmail polling filters for PDF/DBF attachments. CAMS discovery combines configured registrar candidates with the subject-scoped Gmail alternatives `{subject:WBR2 subject:WBR9}` and excludes generic WBR plus attachment/filename discovery terms; the parser still requires subject/body report agreement and supports only WBR2/WBR9, while the post-read Edge sender allowlist remains the security boundary and runs before retrieval. Polling follows at most 4 pages and inspects at most 100 candidates by default, rejects malformed/repeated page tokens, and page-fairly returns at most 25 messages. Page listing is sequential; detail retrieval within each page uses a fixed worker pool (default 5, configurable from 1 through 10), restores Gmail listing order, and cancels/awaits sibling workers on failure. Both provider `attachmentId` and small inline `body.data` MIME parts retain the metadata-only poll/byte-returning fetch split. Inline parts use a bounded, scoped, expiring `inline:<sha256>` locator; only the MIME-part ordinal is cached and fetch re-reads the Gmail message.
* **CAMS Latest-Report Smoke Boundary**: The optional Edge request value `smoke_mode: "latest_supported_reports"` is accepted only for CAMS and maps to two fixed provider queries: configured sender candidates plus `subject:WBR2`, and configured sender candidates plus `subject:WBR9`. Each requests one result. Gmail documents list results as newest first and `internalDate` as the inbox ordering timestamp; selected details still require a valid `internalDate`. The mode therefore reads at most two provider messages/details and returns at most one validated synthesized mailback attachment per report. It never accepts raw query input, never substitutes unsupported reports, preserves post-read sender and subject/body validation, and treats no matches as a bounded no-data smoke completion. Omitting the option preserves the normal backlog path exactly; KFINTECH cannot select the mode.
* **Gmail Detail Response Boundary**: Detail calls use Gmail partial responses to omit unused top-level message metadata while preserving the arbitrary recursive MIME `parts` subtree required by provider `attachmentId` and inline `body.data` attachments. OAuth/list JSON stays under the generic 1 MiB default; message details use a distinct validated 4 MiB maximum, bounding default five-worker raw buffers at 20 MiB without changing attachment-byte limits. Oversized required MIME data fails closed.
* **CAMS Mailback Boundary**: Only WBR2/WBR9 DBF mailbacks with the confirmed Link/validated-URL combination are accepted; unconfirmed or mixed status/URL combinations fail closed. WBR49/unknown reports record `unsupported_report`; exact No Data/NA completes the run with zero attempts. For filename-less CAMS `text/html`/`text/plain` bodies, Gmail `MessagePartBody.size` is validated as bounded integer provider metadata but is not an exact decoded-length checksum; encoded and decoded bytes remain independently bounded, and strict base64url, UTF-8, HTML/WBR, and multipart checks remain. Other Gmail consumers retain exact size matching. Download URLs require HTTPS, exact `mailback<number>.camsonline.com` hosts and `/mailback_result/<opaque>.zip` paths, and reject credentials, ports, queries, fragments, encoded paths, and every redirect. Non-success downloads retain public 502 `provider_request_failed` and may emit only a fixed internal response-class reason for 401/403, 404/410, 429, another 4xx, 5xx, or another unexpected status; numeric provider status, URL, headers, body, cookies, and provider content are excluded. Compressed/uncompressed bytes, time, entries, and expansion ratio are bounded. The encrypted ZIP must contain exactly one safe DBF and is opened in memory with a CAMS-specific provider password; wrong password, corruption, duplicate/traversal/absolute/nested entries, and unsupported methods fail closed. Extracted bytes rejoin the existing integrity, malware, Storage, and DBF-parser pipeline; no new DBF parser or local statement storage is added.
* **CAMS Validation Layers**: Local automated tests prove the complete synthetic HTML-to-validated-URL-to-encrypted-ZIP-to-DBF adapter contract with mocked provider HTTPS. The Hosted Dev synthetic smoke uses the generic Gmail attachment path and existing synthetic DBF fixture to prove the deployed Gmail/Oracle, integrity, malware, Storage, parser, and persistence infrastructure; it does not exercise the CAMS network leg. A genuine WBR2/WBR9 `mailback<number>.camsonline.com` URL and encrypted ZIP remain a separate pending live-provider characterization until an explicitly authorized, appropriately sanitized sample is available. Success of the first two layers is not evidence that the third has passed, and no test hostname or environment bypass is added to the runtime allowlist.
...
* **Deployment Boundary**: Local HTTP is restricted to loopback. The provider-neutral Hosted Dev override removes the API port mapping and exposes only pinned Caddy on 80/443 for automatic HTTPS; ClamAV remains private and signature/TLS state uses named volumes. External secret management is mandatory. The API stays at one worker/replica until Issue #112. Issue #113 is deployed to Hosted Dev and its controlled smoke test has reached Gmail attachment search; final synthetic ingestion awaits Hosted Dev verification of the bounded message-detail response correction. Production is unchanged.
* **Initial OAuth Provisioning Boundary**: An authenticated advisor/admin starts Gmail consent through the Edge Function. It generates 256-bit random state, persists only its SHA-256 digest with actor/workspace/mailbox/exact-redirect binding and a ten-minute expiry, atomically consumes it at callback, and delegates fixed-origin authorization-code exchange to the support service. The configured external redirect URI remains canonical for state binding, Google authorization, and code exchange. At the Hosted Edge gateway boundary, callback routing accepts only the configured `/functions/v1/<function-name>/oauth/callback` pathname and its deterministically derived gateway-stripped `/<function-name>/oauth/callback` runtime pathname; it does not trust Host or forwarded host/protocol headers. The only requested scope is Gmail read-only with offline consent. Access/refresh/expiry values are stored solely in the existing AES-256-GCM mailbox credential envelope using workspace/mailbox/key-version AAD. Reauthorization uses the same fenced upsert; server-side revocation uses a credential-nonce fence and leaves the mailbox `reauthorization_required`.
* **Ingestion Workspace Authorization Boundary**: After Supabase Auth validates the initiating user JWT, ingestion and OAuth start/revoke use the caller's authenticated database context to invoke a boolean-only `SECURITY DEFINER` RPC. The RPC requires one caller profile, an active workspace, and exactly one active, non-ended advisor/admin membership. It has an empty search path, fully qualified references, authenticated-only execution, and does not require direct service-role reads of the protected business tables.
