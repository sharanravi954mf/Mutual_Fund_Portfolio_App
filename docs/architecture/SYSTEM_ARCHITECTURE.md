...
* **Initial OAuth Provisioning Boundary**: An authenticated advisor/admin starts Gmail consent through the Edge Function. It generates 256-bit random state, persists only its SHA-256 digest with actor/workspace/mailbox/exact-redirect binding and a ten-minute expiry, atomically consumes it at callback, and delegates fixed-origin authorization-code exchange to the support service. The only requested scope is Gmail read-only with offline consent. Access/refresh/expiry values are stored solely in the existing AES-256-GCM mailbox credential envelope using workspace/mailbox/key-version AAD. Reauthorization uses the same fenced upsert; server-side revocation uses a credential-nonce fence and leaves the mailbox `reauthorization_required`.
