...
* **Sanitized Support Diagnostics**: Raw Uvicorn access logs remain disabled. The support API emits fixed-schema INFO JSON lines through one non-propagating stdout handler: safe request route/status/duration, successful mailbox message/attachment counts, and sanitized ServiceError code/status. Request/response bodies, headers, provider identities, filenames, senders, content, and credentials are excluded.
