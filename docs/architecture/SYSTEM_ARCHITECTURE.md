...
* **Initial OAuth Provisioning Boundary**: The connector refreshes existing encrypted Gmail credentials, but the consent redirect, callback/state validation, server-side authorization-code exchange, and authorized encrypted first-write path are not yet implemented. Issue #114 owns this prerequisite and must use only Gmail read-only offline access without plaintext or query-string tokens.
