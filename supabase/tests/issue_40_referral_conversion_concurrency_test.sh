...
INSERT INTO auth.users (
  ...
  ('${REFEREE_USER_ID}', 'authenticated', 'authenticated', 'issue40-concurrent-referee@moneybowl.test', '{}', '{"role":"user"}', now(), now());
...
SELECT pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('referral-conversion:${REFEREE_PROFILE_ID}', 0)
);
...
FROM public.process_investor_referral_conversion('${REFERRAL_CODE}');
SQL
}
...
# Hold the processor's identity-derived key. Two independent sessions queue
