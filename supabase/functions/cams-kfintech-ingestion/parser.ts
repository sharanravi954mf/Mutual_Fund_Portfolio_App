...
export type CamsDbfReportSchema =
  | "TRANSACTION_CAPABLE"
  | "NON_TRANSACTION_SCHEMA";

const requiredCamsTransactionAliases: (keyof FieldAliases)[] = [
  "transactionCode",
  "units",
  "amount",
  "nav",
...
export function classifyCamsDbfReport(bytes: Uint8Array): CamsDbfReportSchema {
  const { fields } = decodeDbf(bytes);
  const aliases = fieldAliases.CAMS;
  const transactionCapable = requiredCamsTransactionAliases.every((logical) =>
    fields.some((field) =>
      aliases[logical].some((alias) =>
        alias.toUpperCase() === field.name.toUpperCase()
      )
    )
  );
  return transactionCapable ? "TRANSACTION_CAPABLE" : "NON_TRANSACTION_SCHEMA";
}
...
export class CamsParser extends BaseRegistrarParser {
  ...
  override async parse(input: ParseInput): Promise<NormalizedTransaction[]> {
    if (input.registrar !== this.registrar) {
      throw new IngestionError("unsupported_registrar");
    }
    if (
      input.fileType === "DBF" &&
      classifyCamsDbfReport(input.bytes) !== "TRANSACTION_CAPABLE"
    ) {
      throw new IngestionError("unsupported_statement_format");
    }
