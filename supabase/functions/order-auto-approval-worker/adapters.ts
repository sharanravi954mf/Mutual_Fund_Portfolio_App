  ...
  return {
    ...
    async listRules(input) {
      const { data, error } = await client
        ...
        .select(
          "id, workspace_id, transaction_type, min_amount, max_amount, trusted_client_only, category_restrictions, is_active, rule_version",
        )
    ...
    async recordFailure(input) {
      const { error } = await client.rpc(
        "record_order_auto_approval_event_failure",
        {
          p_event_outbox_id: input.eventOutboxId,
          p_worker_id: input.workerId,
