...
CREATE TABLE IF NOT EXISTS public.mailbox_oauth_credentials (
  ...
  workspace_id pg_catalog.uuid NOT NULL REFERENCES public.workspaces(id) ON DELETE CASCADE,
  access_token_ciphertext pg_catalog.text NOT NULL,
  refresh_token_ciphertext pg_catalog.text,
  nonce pg_catalog.text NOT NULL,
  key_version pg_catalog.int4 NOT NULL DEFAULT 1 CHECK (key_version > 0),
...
BEGIN
  ...
  IF p_mailbox_connection_id IS NULL THEN
    RAISE EXCEPTION 'mailbox_connection_id_required';
  END IF;
  ...
  INSERT INTO public.ingestion_logs (
    ...
    0,
    p_failure_code,
    pg_catalog.jsonb_build_object('failure_code', p_failure_code),
    p_workspace_id,
  ...
  IF p_attachment_sha256 IS NOT NULL
     ...
     AND p_size_bytes IS NOT NULL THEN
...
BEGIN
  ...
  WHERE document.correlation_id = p_correlation_id;
  ...
  IF v_existing_document.id IS NOT NULL THEN
    IF v_existing_document.workspace_id IS DISTINCT FROM p_workspace_id
       ...
       OR v_existing_document.sha256_hex IS DISTINCT FROM p_attachment_sha256 THEN
  ...
  IF v_existing_document.id IS NOT NULL THEN
    IF v_existing_document.mailbox_connection_id IS DISTINCT FROM p_mailbox_connection_id
       OR v_existing_document.registrar IS DISTINCT FROM p_registrar THEN
  ...
  FOR v_tx IN SELECT * FROM pg_catalog.jsonb_array_elements(p_transactions) LOOP
    ...
    IF COALESCE(v_tx ->> 'folioNumber', '') = ''
       ...
       OR COALESCE((v_tx ->> 'amount')::pg_catalog.numeric, 0) <= 0
       OR COALESCE((v_tx ->> 'units')::pg_catalog.numeric, 0) <= 0
       OR COALESCE((v_tx ->> 'nav')::pg_catalog.numeric, 0) < 0 THEN
      RAISE EXCEPTION 'parse_failed';
    END IF;
...
GRANT SELECT, UPDATE ON TABLE public.mailbox_connections TO service_role;
GRANT SELECT, UPDATE ON TABLE public.mailbox_oauth_credentials TO service_role;
