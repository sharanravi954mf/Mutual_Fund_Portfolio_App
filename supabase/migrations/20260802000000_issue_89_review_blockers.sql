-- Issue #89 review blockers: workspace-scoped ingestion and intent-only holdings validation.

BEGIN;

ALTER TABLE public.order_requests
  DROP CONSTRAINT IF EXISTS order_requests_amount_no_nan,
  DROP CONSTRAINT IF EXISTS order_requests_units_no_nan,
  DROP CONSTRAINT IF EXISTS order_requests_amount_finite_positive,
  DROP CONSTRAINT IF EXISTS order_requests_units_finite_positive;

ALTER TABLE public.order_requests
  ADD CONSTRAINT order_requests_amount_finite_positive CHECK (
    amount IS NULL
    OR (
      amount > 0
      AND amount NOT IN ('NaN'::pg_catalog.numeric, 'Infinity'::pg_catalog.numeric, '-Infinity'::pg_catalog.numeric)
    )
  ),
  ADD CONSTRAINT order_requests_units_finite_positive CHECK (
    units IS NULL
    OR (
      units > 0
      AND units NOT IN ('NaN'::pg_catalog.numeric, 'Infinity'::pg_catalog.numeric, '-Infinity'::pg_catalog.numeric)
    )
  );

DROP FUNCTION IF EXISTS public.process_cams_records(pg_catalog.jsonb);
...
BEGIN
  IF p_workspace_id IS NULL OR NOT EXISTS (
    ...
    RAISE EXCEPTION 'process_cams_records_workspace_required';
  END IF;
  ...
  FOR rec IN SELECT * FROM pg_catalog.jsonb_array_elements(records) LOOP
    ...
    SELECT
      pg_catalog.count(*),
      (pg_catalog.array_agg(pfr.portfolio_id ORDER BY pfr.portfolio_id))[1]
