-- Issue #32 integration hardening after PR #90 workspace-isolation merge.

BEGIN;

CREATE OR REPLACE FUNCTION public.assert_active_cams_kfintech_workspace(
...
REVOKE ALL ON FUNCTION public.assert_active_cams_kfintech_workspace(pg_catalog.uuid) FROM PUBLIC, anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.prevent_inactive_cams_kfintech_run_update()
RETURNS pg_catalog.trigger AS $$
BEGIN
  PERFORM public.assert_active_cams_kfintech_workspace(NEW.workspace_id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path = '';

DROP TRIGGER IF EXISTS prevent_inactive_cams_kfintech_run_update
  ON public.cams_kfintech_ingestion_runs;

CREATE TRIGGER prevent_inactive_cams_kfintech_run_update
  BEFORE UPDATE ON public.cams_kfintech_ingestion_runs
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_inactive_cams_kfintech_run_update();

REVOKE ALL ON FUNCTION public.prevent_inactive_cams_kfintech_run_update() FROM PUBLIC, anon, authenticated, service_role;
