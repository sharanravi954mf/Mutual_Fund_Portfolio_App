...
| Provider response amplification | Bounded streamed JSON, message/attachment/part counts, field and identifier limits | Gmail poll performs bounded per-message metadata calls |
...
| OAuth credential leakage | Tokens never logged or persisted by this service; bounded TTL/LRU cache scoped to mailbox identity | Current Edge fetch contract requires a single API replica |
...
| PDF mis-normalization | Registrar marker plus required alias headers; unknown/encrypted layouts fail closed; actual Edge parsers tested | Real layout enablement needs sanitized registrar fixtures |
...
| Cross-mailbox attachment fetch | Fetch requires a fresh token cached for the exact connector/mailbox/registrar identity tuple | Cache state is intentionally process-local |
