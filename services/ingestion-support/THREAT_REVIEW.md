...
| Provider response amplification | Bounded streamed JSON, message/attachment/part counts, field and identifier limits; Gmail detail calls use a partial-response selector plus a distinct 4 MiB maximum while OAuth/list calls retain the generic 1 MiB default; 5 default detail workers bound simultaneous raw detail buffers to 20 MiB (10-worker configuration maximum: 40 MiB) | Arbitrary recursive MIME requires retaining the full `parts` subtree, including inline `body.data`; oversized required MIME data fails closed; detail-worker failures cancel and await siblings |
...
| Sensitive logs | Uvicorn access logs remain disabled; one non-propagating INFO JSON-lines handler emits fixed-schema request, poll-count, and ServiceError events to stdout without bodies, headers, provider identities, filenames, senders, content, or credentials | Host/reverse-proxy logs must also exclude headers and bodies |
